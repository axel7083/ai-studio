 ...
 ***********************************************************************/
import { type Webview, type TelemetryLogger, ImageInfo, containerEngine, ContainerInfo, ContainerInspectInfo, provider, ProviderContainerConnection } from "@podman-desktop/api";
...
const getInitializedInferenceManager = async (): Promise<InferenceManager> => {
  const manager = new InferenceManager(webviewMock, containerRegistryMock, podmanConnectionMock, modelsManager, telemetryMock);
  manager.init();
...
describe('Create Inference Server', () => {
  test('unknown providerId', async () => {
    const inferenceManager = await getInitializedInferenceManager();
    await expect(inferenceManager.createInferenceServer({
      providerId: 'unknown',
...
describe('Start Inference Server', () => {
  test('containerId unknown', async () => {
    const inferenceManager = await getInitializedInferenceManager();
    await expect(inferenceManager.startInferenceServer('unknownContainerId'))
      .rejects.toThrowError('cannot find a corresponding server for container id unknownContainerId.');
  });
...
describe('Stop Inference Server', () => {
  test('containerId unknown', async () => {
    const inferenceManager = await getInitializedInferenceManager();
    await expect(inferenceManager.stopInferenceServer('unknownContainerId'))
      .rejects.toThrowError('cannot find a corresponding server for container id unknownContainerId.');
  });
