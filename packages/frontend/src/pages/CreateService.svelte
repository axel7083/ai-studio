...
$: startedContainerProviderConnectionInfo = $containerProviderConnections.filter(connection => connection.status === 'started');
...
const submit = async () => {
  ...
  if(containerProviderConnection === undefined) throw new Error('containerProviderConnection not defined');
      ...
      <div class="bg-[var(--pd-content-card-bg)] m-5 space-y-6 px-8 sm:pb-6 xl:pb-8 rounded-lg h-fit">
        <div class="w-full">
          ...
          {#if startedContainerProviderConnectionInfo.length > 1}
            <label for="model" class="pt-4 block mb-2 font-bold text-[var(--pd-content-card-header-text)]">Container engine</label>
